...
Util.Fn.extend(Graft.prototype, Model.Relation.Base, {
      ...
      function(clone) {
        ...
        clone._processIncoming().then(
          function() {
            ...
            Util.Fn.each(form.value.find('input'), function(elem) {
              var $elem = CTS.$(elem);
    ...
    if (opp != null) {
      ...
      Util.Promise.all(promises).then(
        function (children) {
          ...
          if (Util.LogLevel.Debug()) {
            Util.Fn.map(replacements, function(r) {
  ...
  clone: function(n1, n2) {
    if (Util.Fn.isUndefined(n1)) {
      n1 = this.node1;
    }
    if (Util.Fn.isUndefined(n2)) {
      n2 = this.node2;
    }
