...
Util._.extend(Graft.prototype, Model.Relation.Base, {
  ...
  // _cloneIterable: function(toward, cloneFromIndex, insertAfterIndex, throwEvent, beforePersistenceFn, cloneRelns, filterFn) {
    ...
    this._cloneIterable(intoList, iterables.length - 1, undefined, false, beforePersistence, true).then(
      function(clone) {
        if (!clone) {
          debugger;
    ...
    clonedChildren.then(
      function(templateCopy) {
        ...
        for (var i = 0; i < templateCopy.length; i++) {
          template.insertChild(templateCopy[0]);
        }
        ...
        Util.Promise.all(
          ...
          function() {
            for (var i = 0; i < templateCopy.length; i++) {
              template.removeChild(templateCopy[i], false);
            }
