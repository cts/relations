...
//    });
  ...
  _findTopmostAreOld: function(node, bestSoFar, callingFrom) {
    ...
    if (r.length > 0) {
      ...
      for (var i = 0; i < r.length; i++) {
        ...
        var idx = Util._.indexOf(node.getIterables(this.otpsFor(node)), callingFrom);
        var iterable = root.getIterables(this.optsFor(root))[idx];
  ...
  _subclass_handleEventFromNode: function(evt, thrownFrom, thrownTo) {
    if (evt.eventName == 'ChildInserted') {
      ...
      var myIterables = thrownTo.getIterables(this.optsFor(thrownTo));
      // TODO YAY!
        ...
        function(clone) {
          ...
          clone.parseInlineRelationSpecsRecursive().then(
            function() {
              ...
              var filterFn = clone.makeIterableLineageFilter(thrownTo, afterIndex+1);  
      ...
      var iterables = thrownTo.getIterables(this.optsFor(thrownTo));
