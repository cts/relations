...
//    });
  ...
  _Are_AlignCardinalities: function(toward) {
    ...
    if (! toOpts.mod) {
      toOpts.mod = toOpts.length;
    }

    var promises = [];

    // Filter the overlap
    for (var i = 0; ((i < fromIterables.length) && (i < toIterables.length)); i++) {
      fromIterables[i].pruneRelations(toward, toIterables[i]);
      toIterables[i].pruneRelations(from, fromIterables[i]);      
    }
    ...
    return Util.Promise.all(clones);
